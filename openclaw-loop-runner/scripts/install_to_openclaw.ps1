...
Write-Host "powershell -ExecutionPolicy Bypass -File `"$targetPlugin\scripts\openclaw_loop.ps1`" -StateDir .openclaw-loop init"
Write-Host "powershell -ExecutionPolicy Bypass -File `"$targetPlugin\scripts\openclaw_loop.ps1`" -StateDir .openclaw-loop plan -Description `"your long task description`""
Write-Host ""
Write-Host "Run/status/check:"
Write-Host "powershell -ExecutionPolicy Bypass -File `"$targetPlugin\scripts\openclaw_loop.ps1`" -StateDir .openclaw-loop status"
Write-Host "powershell -ExecutionPolicy Bypass -File `"$targetPlugin\scripts\openclaw_loop.ps1`" -StateDir .openclaw-loop run"
Write-Host "powershell -ExecutionPolicy Bypass -File `"$targetPlugin\scripts\openclaw_loop.ps1`" -StateDir .openclaw-loop check"
