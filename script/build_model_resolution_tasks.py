...
def render_shared_prompt() -> str:
    ...
    lines = [
        ...
        "",
        "硬门禁：task_json.source_device_pins 是唯一权威 pin 来源。正常情况下，缺少当前 source_part_id pin 列表的对象不会生成 TASK；如果误收到这类 TASK，只能输出 unresolved，不能用框图 source_port/target_port、连线名、规则文本或器件常识生成 selected_pin。",
        "",
        "## 必须遵守",
        "",
        ...
        "15. 如果没有显式 link_info/link_family 数据，且当前 source_part_id 在 source_device_pins 中有非空 pin 列表，必须按 context_group 的源/目的器件类型、源Block名称、源Port 和 mapping_family 继续分析，不得要求用户必须补充链路表。",
